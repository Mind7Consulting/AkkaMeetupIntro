...
object MainObject extends App {
  ...
  // TODO relancer pour observer le comportement. Certain messages vont être envoyé dans le "DeadLetter" office
  val arretSimple = ArretActeurSimple()

  acteurSuperviseur ! messageSimple
  acteurSuperviseur ! messageUrgent
  acteurSuperviseur ! arretSimple
  acteurSuperviseur ! messageSimple
  acteurSuperviseur ! messageUrgent
  // GracefulStop
...
class ActeurEnfant extends Actor {
  ...
  override def postStop(): Unit = {
    println(s"ActeurEnfant : ${self.path} est arrêté")
  }
}
...
class ActeurSuperviseur extends Actor {
  ...
  // TODO ajouter un context.watch pour surveiller les deux acteurs
  context.watch(acteurUrgent)
  context.watch(acteurSimple)
  ...
  override def receive: Receive = {
    ...
    case message: MessageUrgent =>
      ...
      // TODO et le "unwatch"
    case ArretActeurSimple() =>
      // on evoie un message à l'acteur simple pour qu'il s'arrête
      context.become(receiveSolo)
//      context.stop(acteurSimple
//      acteurSimple ! Kill
     acteurSimple ! PoisonPill
      context.unwatch(acteurSimple)
      // et on change de comportement
  ...
  def receiveSolo: Receive = {
    case message: MessageSimple =>
      println(s"Superviseur : reçoit un message simple et le traite seul ${message.text}")
    case message: MessageUrgent =>
      println(s"Superviseur : reçoit un message urgent et l'envoie à son délégué ${message.text}")
      acteurUrgent ! message
  }
}
...
case class ArretActeurSimple()

// TODO : Pour aller plus loin, mettre en place un système de poids dans la mailbox qui permet aux message d'être triés
